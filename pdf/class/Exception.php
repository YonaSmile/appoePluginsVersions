    ...
    final public function __construct($err = 0, $other = null, $html = '')
    {
        ...
        switch($err)
        {
            case 1: // Unsupported tag
                $msg = ( \App\Plugin\Pdf\Locale::get('err01'));
                $msg = str_replace('[[OTHER]]', $other, $msg);
                $this->_tag = $other;
                break;

            case 2: // too long sentence
                $msg = ( \App\Plugin\Pdf\Locale::get('err02'));
            ...
            case 3: // closing tag in excess
                $msg = ( \App\Plugin\Pdf\Locale::get('err03'));
                $msg = str_replace('[[OTHER]]', $other, $msg);
                $this->_tag = $other;
                break;

            case 4: // tags closed in the wrong order
                $msg = ( \App\Plugin\Pdf\Locale::get('err04'));
                $msg = str_replace('[[OTHER]]', print_r($other, true), $msg);
                break;

            case 5: // unclosed tag
                $msg = ( \App\Plugin\Pdf\Locale::get('err05'));
                $msg = str_replace('[[OTHER]]', print_r($other, true), $msg);
                break;

            case 6: // image can not be loaded
                $msg = ( \App\Plugin\Pdf\Locale::get('err06'));
                $msg = str_replace('[[OTHER]]', $other, $msg);
                $this->_image = $other;
                break;

            case 7: // too big TD content
                $msg = ( \App\Plugin\Pdf\Locale::get('err07'));
                break;

            case 8: // SVG tag not in DRAW tag
                $msg = ( \App\Plugin\Pdf\Locale::get('err08'));
                $msg = str_replace('[[OTHER]]', $other, $msg);
                $this->_tag = $other;
                break;

            case 9: // deprecated
                $msg = ( \App\Plugin\Pdf\Locale::get('err09'));
                $msg = str_replace('[[OTHER_0]]', $other[0], $msg);
                $msg = str_replace('[[OTHER_1]]', $other[1], $msg);
                $this->_tag = $other[0];
                break;
        ...
        $this->_messageHtml = '<span style="color: #AA0000; font-weight: bold;">'. \App\Plugin\Pdf\Locale::get('txt01', 'error: ').$err.'</span><br>';
        $this->_messageHtml.=  \App\Plugin\Pdf\Locale::get('txt02', 'file:').' '.$this->file.'<br>';
        $this->_messageHtml.=  \App\Plugin\Pdf\Locale::get('txt03', 'line:').' '.$this->line.'<br>';
        $this->_messageHtml.= '<br>';
        $this->_messageHtml.= $msg;

        // create the text message
        $msg =  \App\Plugin\Pdf\Locale::get('txt01', 'error: ').$err.' : '.strip_tags($msg);
