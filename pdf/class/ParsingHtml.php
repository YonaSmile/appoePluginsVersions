...
class ParsingHtml
{
    ...
    public function parse()
    {
                ...
                if ($res) {
                    ...
                    if (!in_array($res['name'], $tagsNotClosed)) {
                        ...
                        if ($res['close']) {
                            // HTML validation
                            if (count($parents)<1)
                                throw new \App\Plugin\Pdf\Exception(3, $res['name'], $this->getHtmlErrorCode($res['html_pos']));
                            else if ($parents[count($parents)-1]!=$res['name'])
                                throw new \App\Plugin\Pdf\Exception(4, $parents, $this->getHtmlErrorCode($res['html_pos']));
                            else
        ...
        if (count($parents)) throw new \App\Plugin\Pdf\Exception(5, $parents);
