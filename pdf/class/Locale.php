...
class Locale
{
    ...
    static public function load($code)
    {
        ...
        if (!preg_match('/^([a-z0-9]+)$/isU', $code)) {
            throw new \App\Plugin\Pdf\Exception(0, 'invalid language code ['.self::$_code.']');
        }
        ...
        // the file must exist
        if (!is_file($file)) {
            throw new \App\Plugin\Pdf\Exception(0, 'language code ['.self::$_code.'] unknown. You can create the translation file ['.$file.'] and send it to the webmaster of html2pdf in order to integrate it into a future release');
        }
