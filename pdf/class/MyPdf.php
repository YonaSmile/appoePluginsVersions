...
class MyPdf extends \TCPDF
{
    ...
    public function Footer()
    {
        ...
        if ($this->_footerParam['form']) {
            $txt = ( \App\Plugin\Pdf\Locale::get('pdf05'));
        }
        if ($this->_footerParam['date'] && $this->_footerParam['hour']) {
            $txt.= ($txt ? ' - ' : '').( \App\Plugin\Pdf\Locale::get('pdf03'));
        }
        if ($this->_footerParam['date'] && !$this->_footerParam['hour']) {
            $txt.= ($txt ? ' - ' : '').( \App\Plugin\Pdf\Locale::get('pdf01'));
        }
        if (!$this->_footerParam['date'] && $this->_footerParam['hour']) {
            $txt.= ($txt ? ' - ' : '').( \App\Plugin\Pdf\Locale::get('pdf02'));
        }
        if ($this->_footerParam['page']) {
            $txt.= ($txt ? ' - ' : '').( \App\Plugin\Pdf\Locale::get('pdf04'));
        }
    ...
    public function svgPolygone($actions, $style)
    {
        ...
        foreach ($actions as $action) {
            switch($action[0])
            {
                ...
                // Unknown Path
                default:
                    throw new \App\Plugin\Pdf\Exception(0, 'SVG Path Error : ['.$action[0].'] unkown');
            }
