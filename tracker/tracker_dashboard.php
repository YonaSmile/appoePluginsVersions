    ...
    <div class="w-100">
        <strong class="d-block w-100">
            ...
            <div class="d-inline-block mx-1"><input type="date" id="dateStart" class="noBorder"
                                                    max="<?= date('Y-m-d'); ?>" value="<?= $dateStart; ?>"></div>
            <div class="d-inline-block mx-1"><?= trans('Jusqu\'à'); ?> <input type="date" id="dateEnd" class="noBorder"
                                                                              max="<?= date('Y-m-d'); ?>"
                                                                              value="<?= $dateEnd; ?>"></div>
        </strong>
        <div class="my-4">
            <div class="my-2 ms-0 ms-lg-4 position-relative">
                <span class="me-2"><?= trans('Visiteurs'); ?></span>
                <span class="visitsStatsBadge bgColorSecondary"><?= $trackerData['visitors']->ips; ?></span>
            </div>
            <div class="my-2 ms-0 ms-lg-4 position-relative">
                <span class="me-2"> <?= trans('Pages consultées'); ?></span>
                ...
                <div class="nav nav-tabs" role="tablist">
                    <?php foreach ($trackerData['pagesType'] as $type => $count): if ($count == 0) continue; ?>
                        <a class="nav-item sidebarLink colorSecondary nav-link <?= $type === 'PAGE' ? 'active' : ''; ?>"
                           id="nav-<?= $type; ?>-tab"
                           data-bs-toggle="tab" href="#nav-tracker-<?= $type; ?>" role="tab"
                    ...
                    <div class="tab-pane fade <?= $type === 'PAGE' ? ' show active ' : ''; ?>"
                        ...
                        foreach (array_slice($trackerData['pagesName'][$type], 0, 5, true) as $page): ?>
                            <div class="my-2 ms-0 ms-lg-4" style="position: relative;">
                                <span class="me-2"><?= shortenText($page->pageName, 54); ?></span>
