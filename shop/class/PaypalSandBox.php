...
class PaypalSandBox
{
    const PAYPAL_URL = 'https://api-3t.sandbox.paypal.com/nvp?';
    ...
    {
        return array(
            'USER' => 'paypal-facilitator_api1.aoe-communication.com',
            'PWD' => '7TF6U44ZCXC966MN',
            'SIGNATURE' => 'AFcWxV21C7fd0v3bYYYRCpSSRl31AMpdydkFkv.GWVHph0f4FiQqKvFj',
            'VERSION' => '204.0'
        );
    }
    ...
    {
        //prepare query
        $query = http_build_query($this->getAuthApi(), '', '&')
            . '&'
            . http_build_query($this->request, '', '&');

        //send query
        $ch = curl_init(self::PAYPAL_URL);
        ...
        if (!$this->response) {
            ...
            if ($this->listParam['ACK'] == 'Success') {
                ...
                $this->errorPaypal = 'Erreur de communication avec le serveur PayPal. ' . $this->listParam['L_LONGMESSAGE0'];

                //TODO Annuler commande en cours. (sinon s'annule toute seule en 10 min)
