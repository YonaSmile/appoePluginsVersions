...
<?= getTitle($Page->getName(), $Page->getSlug()); ?>
                        ...
                        <?php if ($allEmployes): ?>
                            <?php foreach ($allEmployes as $employe): ?>
                                <tr data-idemploye="<?= $employe->id ?>">
                                    ...
                                    <td>
                                        <?php
                                        $allContrats = getEmployeContrats($employe->id);
                                        foreach ($allContrats as $idContrat => $contrat): ?>
                                            <div class="btn-group" data-idcontrat="<?= $contrat['id']; ?>">
                                                <button type="button" data-toggle="popover" data-placement="top"
                                                        class="btn btn-sm btn-outline-secondary"
                                                        data-content="<strong>Depuis</strong> <?= displayCompleteDate($contrat['dateDebut']) . '<br><strong>' . $contrat['typeContrat'] . '</strong> ' . $contrat['nbHeuresSemaines'] . 'h/semaines'; ?>">
                                                    <?= $contrat['siteName']; ?>
                                                </button>
                                                <button type="button" aria-label="Close"
                                                        data-idcontrat="<?= $contrat['id']; ?>"
                                                        class="btn btn-sm btn-outline-secondary dropdown-toggle-split removeContrat">
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.removeContrat').on('click', function (event) {
                event.stopImmediatePropagation();
                event.preventDefault();
                var $btn = $(this);

                if (confirm('Vous allez archiver ce contrat !')) {
                    var idContrat = $btn.data('idcontrat');
                    busyApp();
                    $.post(
                        '<?= AGAPESHOTES_URL . 'process/ajaxEmployesProcess.php'; ?>',
                        {
                            ARCHIVEEMPLOYECONTRAT: 'OK',
                            idContrat: idContrat
                        },
                        function (data) {
                            if (data === true || data == 'true') {
                                $btn.parent('div.btn-group').fadeOut();
                            }
                            availableApp();
                        }
                    );
