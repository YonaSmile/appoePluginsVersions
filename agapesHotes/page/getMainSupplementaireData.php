...
if ($Secteur->showBySlug() && $Site->showBySlug() && $Site->getSecteurId() == $Secteur->getId()):
    ...
    $allMainsSupp = $MainSupp->showByDate($start->format('Y-m-d'), $end->format('Y-m-d'));
    ?>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <form action="" method="post"
                        ...
                        <div class="col-12 text-right totalContainer-<?= $clientNameSlug; ?>">
                            <?php
                            $allTtcTvaTotal = array();
                            foreach ($allTvaTotal as $tva => $tvaData):
                                $totalTva = 0;
            ...
            function updateFormAddProductTotal() {
                ...
                $('.allFactureProducts').find('.productFields').each(function (i) {
                    ...
                    if (quantite > 0 && prixUnitaire > 0) {
                        ...
                        $parent.find('input.totalField').val(0);
                        $parent.find('input.totalTtcField').val(0);
                    }
                });
            }
                ...
                $.post(
                    ...
                    function (data) {
                        if (data === true || data == 'true') {
                            $('.modal-backdrop').hide();
                            $('#MainSuppContainer').html(loaderHtml());
                            setTimeout(function () {
