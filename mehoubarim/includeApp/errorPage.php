...
if (in_array('app', $mehoubarim_url_parts) && in_array('page', $mehoubarim_url_parts)) {
    ...
    if (true !== $mehoubarim && false !== $mehoubarim) {
        $message = trans('Cette page est en ce moment manipulé par') . ' <strong>' . getUserEntitled(ShinouiKatan::Decrypter($mehoubarim)) . '</strong>';

        if (getConfig('sharingWork') === 'true') {

            define('MEHOUBARIM_MSG', 'Cette page est en ce moment manipulé par ' . getUserEntitled(ShinouiKatan::Decrypter($mehoubarim)));
